...
class cleanto_setting
{
	...
	public $ct_allow_customer_cancel;
	public $ct_allow_customer_reschedule;
	public $ct_allow_manual_booking;
	public $ct_currency;
	...
	{
		$this->option_name = mysqli_real_escape_string($this->conn, $option_name);
		$this->option_value = mysqli_real_escape_string($this->conn, $option_value);

		$chk_query = "select `id` from `" . $this->table_name . "` where `option_name`='" . $this->option_name . "'";
		$chk_res = mysqli_query($this->conn, $chk_query);
	...
	public function chk_epc($settings, $conn)
	{
		...
		$pc = isset($_POST['purchase_code']) ? $_POST['purchase_code'] : '';
		$ext_pstatus = "ct_" . base64_encode($ext . "_pstatus");
		$chk_pstatus_option = $settings->get_option($ext_pstatus);
		$ext_pcode = "ct_" . $ext . "_purchase_code";
		$get_pcode = $settings->get_option($ext_pcode);
		$codeForCheck = !empty($get_pcode) ? $get_pcode : $pc;
		$mixedstring = substr($codeForCheck, -4) . 'sm';
		...
		if (!empty($pc)) {
			$settings->set_option($ext_pcode, $pc);
			$settings->set_option($ext_pstatus, $checkstring);
			echo "valid";
		} else {
			echo "invalid";
		}
