<?php
/**
 * Cronjob: Sync Reservations from Google Calendar into Local System & Kinesis API
 * 
 * Usage from CLI / Crontab:
 * php /path/to/cron/sync_gcal_to_system.php
 * Or via web request (secured):
 * https://your-domain.com/cron/sync_gcal_to_system.php?key=YOUR_CRON_OR_API_KEY
 */
...
require_once dirname(dirname(__FILE__)) . '/integrations/awwapi/AwwGCalSync.php';
...
$gcStatus = $setting->get_option('ct_gc_status');
if ($gcStatus !== 'Y') {
    $msg = 'Google Calendar sync is currently disabled in settings.';
    if ($isCli) {
        fwrite(STDERR, $msg . "\n");
        exit(1);
    }
